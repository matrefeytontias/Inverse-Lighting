...
vec3 brdf(vec3 v, vec3 l, vec3 n)
{
    ...
    vec3 cdiff = mix(albedo * (1. - dielectricSpecular.r), black, metalRough.r),
...
void main()
{
    ...
    fragColor = texture(uEmissiveMap, vTexCoord).rgb + 20. / (1. + dot(vRay, vRay)) * brdf(v, v, n) * texture(uOcclusionMap, vTexCoord).r
        * max(0., -dot(n, v));
}
